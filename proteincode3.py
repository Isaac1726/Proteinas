...
Original file is located at
    https://colab.research.google.com/drive/1n4cqQaNKoRAH0rg5P5RmPaJEOzhZ3ht_
"""
...
# Carga información adicional desde la API de la PDB
@st.cache_data
def fetch_pdb_additional_info(pdb_id):
...
pdb_info = fetch_pdb_additional_info(selected_protein)

if pdb_info:
    st.subheader("Información adicional desde la PDB:")
    st.write(f"**Título de la estructura:** {pdb_info.get('struct', {}).get('title', 'No disponible')}")
    st.write(f"**Método de resolución:** {pdb_info.get('exptl', [{}])[0].get('method', 'No disponible')}")
    st.write(f"**Resolución:** {pdb_info.get('rcsb_entry_info', {}).get('resolution_combined', ['No disponible'])[0]} Å")
    st.write("**Referencias:**")
    for reference in pdb_info.get('rcsb_primary_citation', {}).get('rcsb_authors', []):
        st.write(f"- {reference}")
else:
    st.write("No se pudo obtener información adicional desde la PDB.")
...
# Indicador 2: Gráfico 3D de residuos con fondo blanco
...
# Indicador 3: Distribución de elementos
...
# Indicador 4: Visualización de la proteína (corregido)
st.subheader("Visualización de la proteína:")
background_color = st.color_picker("Selecciona el color del fondo:", "#ffffff")

view = py3Dmol.view(width=900, height=600)
pdb_data = ppdb.pdb_text  # Se utiliza directamente el texto de la proteína
view.addModel(pdb_data, "pdb")
view.setStyle({"cartoon": {"color": "spectrum"}})
view.setBackgroundColor(background_color)
view.zoomTo()

# Generar HTML para mostrar en Streamlit
protein_html = view._make_html()
st.components.v1.html(protein_html, height=600)